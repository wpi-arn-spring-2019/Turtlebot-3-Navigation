...
{
    m_pose_sub = nh.subscribe<geometry_msgs::PoseWithCovarianceStamped>("/pf_pose", 10, &OdomCorrection::poseCallback, this);
}
...
void OdomCorrection::correctOdom()
{
    if(m_have_pose)
    {
        ...
        tf::Point pt;
        tf::pointMsgToTF(m_pose->pose.pose.position, pt);
        tf::Quaternion q;
        tf::quaternionMsgToTF(m_pose->pose.pose.orientation, q);
...
void OdomCorrection::poseCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &msg)
{
    if(!m_have_pose)
    {
        m_have_pose = true;
